...
std::vector<shader::pointLight> generatePointLights(nvmath::vec3 min,
                                                    nvmath::vec3 max) {
...
std::vector<shader::triangleLight> collectTriangleLights(
    const nvh::GltfScene& scene) {
  ...
  for (const nvh::GltfNode& node : scene.m_nodes) {
    ...
    if (material.emissiveFactor.sq_norm() > 1e-6) {
      ...
      for (uint32_t i = 0; i < mesh.indexCount; i += 3, indices += 3) {
        ...
        shader::triangleLight tmpTriLight{
            p1, p2, p3, vec4(material.emissiveFactor, 0.0), area};
...
[[nodiscard]] std::vector<shader::aliasTableCell> createAliasTable(
  ...
  std::vector<shader::aliasTableCell> aliasTable(
      lightNum, shader::aliasTableCell{-1, 0.f, 0.f});
