...
Camera::~Camera() {
  if (has_device_memory_) {
    vkUnmapMemory(metadata_.context->GetDevice(), buffer_memory_);
    vkDestroyBuffer(metadata_.context->GetDevice(), buffer_, nullptr);
    vkFreeMemory(metadata_.context->GetDevice(), buffer_memory_, nullptr);
  } else {
    spdlog::info(
...
void Camera::CreateDeviceBuffer() {
  ...
  vkMapMemory(metadata_.context->GetDevice(), buffer_memory_, 0,
