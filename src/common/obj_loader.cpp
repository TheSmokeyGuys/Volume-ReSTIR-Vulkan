...
void ObjLoader::loadModel(const std::string& filename) {
  ...
  for (const auto& material : reader.GetMaterials()) {
    MaterialObj m;
    m.ambient  = nvmath::vec3f(material.ambient[0], material.ambient[1],
                              material.ambient[2]);
    m.diffuse  = nvmath::vec3f(material.diffuse[0], material.diffuse[1],
                              material.diffuse[2]);
    m.specular = nvmath::vec3f(material.specular[0], material.specular[1],
                               material.specular[2]);
    m.emission = nvmath::vec3f(material.emission[0], material.emission[1],
                               material.emission[2]);
    m.transmittance =
        nvmath::vec3f(material.transmittance[0], material.transmittance[1],
  ...
  if (m_materials.empty()) m_materials.emplace_back(MaterialObj());
  ...
  for (auto& mi : m_matIndx) {
    if (mi < 0 || mi > m_materials.size()) mi = 0;
  }
